...
use App\Services\PerplexityCompanyService;
...
class ScrapController extends Controller
{
    ...
    // ✅ Add this
    protected PerplexityCompanyService $perplexityCompanyService;

    public function __construct(PerplexityCompanyService $perplexityCompanyService)
    {
        $this->perplexityCompanyService = $perplexityCompanyService;
    }
        ...
        foreach ($jobs as $jobIndex => $job) {
            ...
            try {
                ...
                if (!$office) {
                    ...
                    $perplexityData = $this->perplexityCompanyService->findCompanyData($job);

                    Log::info('Perplexity Result', [
                        'data' => $perplexityData
                    ]);

                    $companyWebsite = $perplexityData['company_website'] ?? null;
                    ...
                    // ✅ Correct - map flat perplexity response to a single contact
                    if (!empty($perplexityData)) {
                        Contact::create([
                            'contactable_id'   => $office->id,
                            'contactable_type' => Office::class,
                            'contact_name'     => $perplexityData['contact_person'] ?? $companyName,
                            'contact_email'    => $perplexityData['email'] ?? null,
                            'contact_phone'    => $perplexityData['phone'] ?? null,
                            'contact_landline' => $perplexityData['landline'] ?? null,
                        ]);
                    }
                }
    ...
    public function getScrappedSales(Request $request)
    {
        ...
        <iconify-icon icon="solar:square-arrow-right-up-bold" class="text-info fs-24"></iconify-icon>
                    ...
                    if ($sale->office_name) {
                        $prompt = urlencode('give me the contact information about this company "' . $sale->office_name . '" uk official data like phone, email, contact person and website url');

                        $output .= ' <a href="https://www.perplexity.ai/search?q=' . $prompt . '" target="_blank" title="Search on Perplexity">
            <iconify-icon icon="simple-icons:perplexity" class="text-info fs-24"></iconify-icon>
        </a>';

                        $output .= ' <a href="https://chatgpt.com/?q=' . $prompt . '" target="_blank" title="Search on ChatGPT">
            <iconify-icon icon="simple-icons:openai" class="text-success fs-24"></iconify-icon>
        </a>';
                    }
