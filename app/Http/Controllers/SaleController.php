        ...
        if ($request->ajax()) {
            return DataTables::eloquent($model)
                ...
                ->addColumn('cv_limit', function ($sale) {
                    $status = $sale->no_of_sent_cv == $sale->cv_limit ? '<span class="badge w-100 bg-danger" style="font-size:90%" >' . $sale->no_of_sent_cv . '/' . $sale->cv_limit . '<br>Limit Reached</span>' : "<span class='badge w-100 bg-primary' style='font-size:90%'>" . ((int)$sale->no_of_sent_cv - (int)$sale->cv_limit. '/' . (int)$sale->cv_limit) . "<br>Limit Remains</span>";
                    return $status;
                })
