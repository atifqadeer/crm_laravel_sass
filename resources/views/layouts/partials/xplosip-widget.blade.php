    ...
    (function() {
            ...
            try {
                fetch(XP_ACQUIRE_URL, {
                    ...
                    if (res.status === 200 && res.body && res.body.ok) {
                        ...
                        cb({
                            ok: false,
                            reason: (res.body && res.body.reason) || (res.body && res.body
                                .lockedBySelf ? 'self_lock' : 'other_lock'),
                            ...
                            message: (res.body && res.body.message) ||
                                'This number is currently in use.',
                        });
                    }
                }).catch(function() {
                    cb({
        ...
        function showLocked(overlay, info) {
            ...
            if (reason === 'daily_limit') {
                ...
                subline = "You've called this number " + info.dailyCallCount + '/' + info.dailyCallLimit +
                    ' times today.';
            ...
            card.innerHTML =
                ...
                '<div class="xpc-lock-bar"><div class="xpc-lock-bar-fill ' + styleClass +
                '" id="xpc-bar" style="width:100%"></div></div>' +
                '<div class="xpc-lock-label">' + timerLabel + '</div>' +
                '<div class="xpc-lock-timer ' + styleClass + '" id="xpc-timer">' + fmtSeconds(remaining) +
                '</div>' +
                '</div>' +
                (info.callCount > 0 ? '<div style="font-size:11px;color:#94a3b8;margin-bottom:14px">Called ' + info
                    .callCount + '× in total</div>' : '') +
        ...
        window.xplosipDial = function(number, maskedDisplay) {
            if (!number) return;
            var num = String(number).trim();
            var display = maskedDisplay ? String(maskedDisplay).trim() : num;
            closeConfirm();
            ...
            overlay.innerHTML =
                ...
                '</div>' +
                '</div>';
            overlay.querySelector('.xpc-num').textContent = display;
            ...
            fetchInfo(num, function(info) {
                ...
                if (info.locked) {
                    ...
                    meta.textContent = isSelf ?
                        ...
                        '⚠ In use by ' + (info.lockedBy || 'another agent') + ' — free in ' +
                        fmtSeconds(info.remainingSeconds);
                    callBtn.disabled = true;
                    callBtn.style.opacity = '0.5';
                    callBtn.style.cursor = 'not-allowed';
                    callBtn.title = isSelf ? 'Your re-dial lock is active' :
                        'Locked — being called by another agent';
                    ...
                    var preCountdown = setInterval(function() {
                        ...
                        if (info.remainingSeconds <= 0) {
                            ...
                            meta.textContent = isSelf ?
                                '⏳ Your re-dial lock — free in ' + fmtSeconds(info
                                    .remainingSeconds) :
                                '⚠ In use by ' + (info.lockedBy || 'another agent') +
                                ' — free in ' + fmtSeconds(info.remainingSeconds);
                        }
                    }, 1000);
                } else if (info.dailyLimitReached) {
                    meta.className = 'xpc-meta warn';
                    meta.textContent = '🚫 Daily limit reached (' + info.dailyCallCount + '/' + info
                        .dailyCallLimit + ') for this number — resets in ' + fmtSeconds(info
                            .dailyResetSeconds);
                    callBtn.disabled = true;
                    callBtn.style.opacity = '0.5';
                    callBtn.style.cursor = 'not-allowed';
                    ...
                    if (info.dailyCallLimit > 0) parts.push((info.dailyCallCount || 0) + '/' + info
                        .dailyCallLimit + ' today');
                    meta.className = 'xpc-meta';
